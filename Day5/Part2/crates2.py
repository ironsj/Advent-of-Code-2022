...
with open('../input.txt', 'r') as f:
    ...
    # for each set of instructions, find how many crates must be moved, take them off the appropriate stack and append to temporary list
    for numbered_instructions in simplified:
        moving_crates = []
        for _ in range(numbered_instructions[0]):
            moving = stacks[numbered_instructions[1] - 1].pop()
            moving_crates.append(moving)
        # take off of top of temporary list (the crate that was lowest one being moved) and add to new stack
        for _ in range(len(moving_crates)):
            adding = moving_crates.pop()
            stacks[numbered_instructions[2] - 1].append(adding)
